...
def write_human_report(account: str, report: dict) -> None:
    ...
    # ── Engagement Bot ─────────────────────────────────────────────────────────
    eng_log_path = os.path.join(TMP_BASE, account, "engagement_log.json")
    lines.append("## Engagement Bot — Reply Activity")
    if os.path.exists(eng_log_path):
        try:
            with open(eng_log_path, "r", encoding="utf-8") as _f:
                eng_log = json.load(_f)
            replies      = eng_log.get("replies", [])
            total_sent   = sum(1 for r in replies if r.get("status") == "sent")
            today_str    = datetime.date.today().isoformat()
            sent_today_e = sum(1 for r in replies if r.get("status") == "sent"
                               and r.get("timestamp", "")[:10] == today_str)
            # comments received = unique comment_ids we saw (sent + skipped spam)
            seen_media   = {r["media_id"] for r in replies if r.get("media_id")}
            # turnaround: time between comment timestamp and reply timestamp (unavailable
            # without comment ts — skip if not stored)
            # reply-to-comment ratio: sent / total comments seen per media
            mentions_rep = sum(1 for r in replies if r.get("is_mention") and r.get("status") == "sent")
            last_run     = eng_log.get("stats", {}).get("last_run", "never")[:16]
            lines.append(f"- Total replies sent (all time): **{total_sent}**")
            lines.append(f"- Replies sent today: **{sent_today_e}**")
            lines.append(f"- @mention replies (all time): {mentions_rep}")
            lines.append(f"- Posts with reply activity: {len(seen_media)}")
            lines.append(f"- Last cycle ran: {last_run}")
        except Exception:
            lines.append("_(engagement log present but could not be read)_")
    else:
        cfg_eng = _load_config(account).get("engagement", {})
        if cfg_eng.get("enabled", False):
            lines.append("_(engagement bot enabled — log will appear after first cycle)_")
        else:
            lines.append("_(engagement bot disabled — set `engagement.enabled = true` in config to activate)_")
    lines.append("")
